// 170101011-live-yes-no.cpp : Defines the entry point for the console application.
//
#include "stdafx.h"
#include <iostream>
#include <fstream>
...
#define duration 12
#define recmod "Recording_Module.exe"
...
string inpwav= "inp.wav",inptxt = "inp.txt";
...
int main()
{
		...
		oss << recmod << " "<<duration <<" "<< inpwav << " " << inptxt; //define the string manually, because macro cant expand inside string
		system(oss.str().c_str()); //call the system function to read directly from the mic
		filename=inptxt; //name of the recorded speech text file

		InpSpeech.open("yes_no.txt", ios::in); // open the file to read from it
		...
		double AvgZCR[400], AvgEnergy[400];
		for(i=0;i<framecount;i++)
		{
			...
			for(j=0;j<framesize-1;j++)
			{
				if((samples[i*framesize + j]-DCshift)*Ampscale/MaxAmp*((samples[i*framesize + j + 1]-DCshift)*Ampscale*1.0 < 0)) // If two adjacent samples have opposite sign then increment the avergae ZCR by 1
					AvgZCR[i]+=1;
				AvgEnergy[i]+=(double)1.0*(samples[i*framesize + j]-DCshift)*Ampscale/MaxAmp*(samples[i*framesize + j]-DCshift)*Ampscale/MaxAmp; // energy is calculated as square of amplitude of the sample value
			}
			//out << "ZCR in "<<i+1<<"th frame is "<< AvgZCR[i] << "\n";
			AvgZCR[i]/=framesize;
			//out << "Avergae ZCR in\t\t "<<i+1<<"th frame is "<< AvgZCR[i] << "\n";
			AvgEnergy[i]+=(double)1.0*(samples[i*framesize + j]-DCshift)*Ampscale/MaxAmp*(samples[i*framesize + j]-DCshift)*Ampscale/MaxAmp; // average energy for all the frames after scaled sample values
		...
		ThresholdZCR=(double)TotalZCR/InitFrames;
		NoiseEnergy/=InitFrames;
		ThresholdZCR*= (double)0.9;
